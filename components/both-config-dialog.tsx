...
export function BothConfigDialog({ open, onOpenChange, onSelect }: BothConfigDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] sm:max-w-[90vw] md:max-w-[85vw] lg:max-w-[1600px] w-[95vw] sm:w-[90vw] md:w-[85vw] lg:w-[1200px] max-h-[95vh] overflow-hidden flex flex-col p-4 sm:p-5 md:p-6 pr-10 sm:pr-12">
        <DialogHeader className="pb-3 space-y-2 pr-6 sm:pr-8">
          <DialogTitle className="text-lg sm:text-xl font-semibold leading-tight">Select BOTH (DCR + NON DCR) Configuration</DialogTitle>
              ...
              <Table className="w-full">
                <TableHeader className="sticky top-0 z-30">
                  <TableRow className="bg-primary/10 hover:bg-primary/10 border-b">
                    ...
                    <TableHead className="font-semibold text-sm px-4 sm:px-5 md:px-6 py-3 sm:py-3.5 whitespace-nowrap bg-primary/10">DCR Capacity</TableHead>
                    <TableHead className="font-semibold text-sm px-4 sm:px-5 md:px-6 py-3 sm:py-3.5 whitespace-nowrap bg-primary/10">NON DCR Capacity</TableHead>
                ...
                {filteredConfigs.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center text-muted-foreground py-8 sm:py-10 text-sm">
                  ...
                  filteredConfigs.map((config, index) => (
                    <TableRow key={index} className="hover:bg-muted/50 border-b transition-colors">
                      ...
                      <TableCell className="text-sm px-4 sm:px-5 md:px-6 py-3 sm:py-4 whitespace-nowrap text-green-600 font-semibold">
                        {config.dcrCapacity}
                      </TableCell>
                      <TableCell className="text-sm px-4 sm:px-5 md:px-6 py-3 sm:py-4 whitespace-nowrap text-blue-600 font-semibold">
